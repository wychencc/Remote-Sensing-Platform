...
#     # print(img_cld.cpu()[0].shape)
#
#     # 压缩维度 转NUMPY 转维度 乘以缩放比 再转int8
#     # 转换之后维度分别为 256*256*15   256*256*15  256*256*15  256*256*1
#     img_cld = uint16to8((img_cld.cpu()[0].numpy() * scale).astype("uint16")).transpose(1, 2, 0)
#     # print(1)
#     # print(img_cld.shape)
#     img_fake = uint16to8((img_fake.cpu()[0].numpy() * scale).astype("uint16")).transpose(1, 2, 0)
#     # print(img_fake)
#     # print(img_fake.shape)
#     img_truth = uint16to8((img_truth.cpu()[0].numpy() * scale).astype("uint16")).transpose(1, 2, 0)
#
#     img_csm = img_csm[0].cpu().numpy().transpose(1, 2, 0)
#     # print(img_cld.shape,img_fake.shape,img_truth.shape)
#     # 取RGB
#     img_cld_RGB = getRGBImg(img_cld[:, :, 8], img_cld[:, :, 7], img_cld[:, :, 6], img_size)
#     # img_sar_RGB =  getRGBImg(img_cld[:, :, :], img_cld[:, :, 4], img_cld[:, :, 3], img_size)
#     # img_edge_RGB =  getRGBImg(img_cld[:, :, 2], img_cld[:, :, 1], img_cld[:, :, 0], img_size)
#
# # 有问题
...
#     # output_img[:, 4 * img_size:5 * img_size, :] = img_sar
#     # output_img[:, 5 * img_size:6 * img_size, :] = img_edge
#
#     return output_img
