...
def __getitem__( s1_path,s2_path,s2_cloud_path):
        # 读取
        sar_path = s1_path
...
def predict(config,model_path,s1_path,s2_path,s2_cloud_path):
    ...
    # 数据集
    #inputdata, s2CSMimg, img_rgb, img_sar, img_cloud = __getitem__(s1_path,s2_path,s2_cloud_path)
    #img_cld,img_csm,img_truth, img_sar,img_cloud= __getitem__(s1_path,s2_path,s2_cloud_path)
    #print("数据集初始化完毕")
    ...
    with t.no_grad():       
        ...
        print("*******")
        psnr_4 = PSNR(img_fake,img_truth)
        print("psnr_4")
        ssim_4 = SSIM(img_fake,img_truth)
        print("ssim_4")
        pixel_errors = MAE(img_fake,img_truth, False)
        print("pixel_errors")
        mae_4 = pixel_errors.mean()
        angles = SAM(img_fake,img_truth, False)
        sam_4 = angles.mean()
        print("sam_4")
        PSNR_4 += psnr_4
        SSIM_4 += ssim_4
        ...
        plt.subplot(142)
        plt.title('Cloudy')
        occ =img_cloud.cpu()[0].permute(1, 2, 0)
        occ = occ - occ.min()
        occ = occ / occ.max()
        plt.imshow(occ)
        ...
        plt.subplot(143)
        plt.title('Target')
        rgb = img_truth.cpu()[0].permute(1, 2, 0)
        rgb = rgb - rgb.min()
        rgb = rgb / rgb.max()
        plt.imshow(rgb)
...
predict(myconfig,model_path,s1_path,s2_path,s2_cloud_path)
