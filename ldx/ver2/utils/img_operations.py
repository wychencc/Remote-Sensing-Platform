...
def carl_error(y_true, csm,y_pred):
    ...
    #     predicted - target))
    loss1 = 1.0 * t.mean(t.abs(predicted - target))
    loss2 =  t.mean(clearmask * t.abs(predicted - input_cloudy) + csm * t.abs(
        predicted - target))
    ...
    log = 'Time:{}'.format(time) + '/' + 'loss:' + str(loss.item()) + '/' + 'loss1:' + str(
        loss1.item())+ '/' + 'loss2:' + str( loss2.item())
    ...
    # sar = canny(y_pred).detach().cpu().numpy()[0][0]
    # # sar = sar - sar.min()
    # # sar = sar / sar.max()
    # # print(sar.shape)
    # plt.imshow(sar)
    ...
    # occ = canny(y_true).detach().cpu().numpy()[0][0]
    # # occ = occ - occ.min()
    # # occ = occ / occ.max()
    # plt.imshow(occ)
    # plt.show()
    with open('./loss.txt', 'r+', newline='', encoding='utf-8') as f:
        f.seek(0, 2)
        f.write('\r' + log)
    return loss
